...
def get_role_id(db: Session, role_name: str) -> int:
    ...
    if not role:
        logger.error(f"CRITICAL: Role '{role_name}' not found in the database! Check seed data.")
        raise HTTPException(
...
def register_customer(db: Session, customer_data: schemas.CustomerRegister):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="An internal server error occurred while creating your account. Please try again later."
        )
...
def register_airline(db: Session, airline_data: schemas.AirlineRegister):
    ...
    try:
        ...
        db.rollback() 
        logger.error(f"Database error during airline registration: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="An internal server error occurred while creating the airline account. Please try again later."
        )
...
def authenticate_user(db: Session, form_data: OAuth2PasswordRequestForm):
    ...
    try:
        ...
        db.rollback() 
        logger.error(f"Database error during user login (LastLoginAt update): {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="An internal server error occurred while processing your login. Please try again later."
        )
